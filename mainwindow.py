...
class MainWindow(QMainWindow):
    def __init__(self, *args):
        ...
        self.customContextMenuRequested.connect(self.contextMenu)
        self.popMenu = QMenu(self)
        self.setWindowTitle("简易CAD软件(采用PythonOCC-%s)"%VERSION)
        self.resize(960, 720)
    ...
    def createDockWidget(self):
        self.treeDockWidget = QDockWidget("组件区", self)
    ...
    def setItemActive(self, item):
        ...
        if item:
            ...
            if uid in pd:
                ...
                sbText = "%s [uid=%i] 成为当前组件" % (name, uid)
                self.redraw()
            elif uid in wd:
                self.setActiveWp(uid)
                sbText = "%s [uid=%i] 成为当前工作平面" % (name, uid)
                self.redraw()
            elif uid in ad:
    ...
    def registerCallback(self, callback):
        ...
        self.currOpLabel.setText("当前操作: %s " % callback.__name__[:-1])
    ...
    def clearCallback(self):
        if self.registeredCallback:
            ...
            self.registeredCallback = None
            self.clearAllStacks()
            self.currOpLabel.setText("当前操作: 无 ")
